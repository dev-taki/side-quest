...
export class AdminAuthService {
  ...
  static async getAdminProfile(): Promise<AdminProfile> {
    return this.apiRequest<AdminProfile>('/auth/me');
  }

  // Verify admin status
  static async verifyAdminStatus(): Promise<boolean> {
    try {
      ...
      return profile.role === 'admin' || profile.role === 'super_admin' || profile.role === 'owner';
    } catch (error) {
      return false;
    }
  }
